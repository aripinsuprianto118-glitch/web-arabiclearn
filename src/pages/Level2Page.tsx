...
export default function Level2Page({
  ...
  const allWords = [...q.correctOrder, ...q.distractors].sort(
    () => Math.random() - 0.5,
  );

  const bg = darkMode ? "bg-[#080c18]" : "bg-slate-50";
  const card = darkMode
    ? "bg-[#0f1629] border-white/8"
  ...
  function handleFinish() {
    const xp =
      Math.round((correctCount / total) * 60) +
      (correctCount === total ? 20 : 0);
    onComplete(xp);
    ...
    return (
      <div className={`min-h-screen ${bg} geometric-bg`}>
        ...
        <div className="pt-28 pb-16 px-4 max-w-md mx-auto flex flex-col items-center text-center">
          ...
          <h2 className={`text-3xl font-black ${text} mb-2`}>
            {tr(lang, "levelComplete")}
          </h2>
          <p className={`${subtext} mb-8`}>
            {lang === "id"
              ? "Kuis Level 2 — Menyusun Kalimat Selesai!"
              : "Level 2 Sentence Building Complete!"}
          </p>
          <div className={`w-full ${card} border rounded-2xl p-6 mb-6`}>
            ...
            <div className="flex justify-center gap-2 mt-5">
              {[1, 2, 3].map((s) => (
                <span
                  key={s}
                  className={`text-3xl ${pct >= s * 34 ? "" : "grayscale opacity-30"}`}
                >
                  ⭐
                </span>
              ))}
      ...
      <div className="pt-24 pb-16 px-4 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-4 animate-fade-in-up">
          <button
            ...
            className={`text-sm font-semibold ${subtext} hover:text-amber-500 transition-colors`}
          >
            ✕
          </button>
        ...
        <div
          className={`${card} border rounded-2xl p-5 mb-4 animate-fade-in-up`}
        >
          <div className="flex items-start gap-4">
            <div className="flex-1 min-w-0">
              <p
                className={`text-xs font-bold uppercase tracking-wider ${subtext} mb-1`}
              >
                {tr(lang, "instruction")}
              </p>
        ...
        {/* Mode toggle */}
        <div
          className={`${card} border rounded-2xl p-1 mb-4 flex animate-fade-in-up`}
        >
          <button
            onClick={() => {
              setMode("click");
        ...
        {mode === "click" ? (
          <>
            {/* Answer area */}
            <div
              className={`${card} border rounded-2xl p-5 mb-4 min-h-16 animate-fade-in-up`}
            >
              <div className="flex items-center justify-between mb-3">
                <p
                  className={`text-xs font-bold uppercase tracking-wider ${subtext}`}
                >
                  {tr(lang, "yourAnswer")}
                </p>
                <button
                  onClick={clearAll}
                  className={`text-xs font-semibold ${subtext} hover:text-red-400 transition-colors`}
                >
                  {tr(lang, "clearAll")} ✕
                </button>
              </div>
            ...
            >
              <p
                className={`text-xs font-bold uppercase tracking-wider ${subtext} mb-3`}
              >
                {tr(lang, "availableWords")}
              </p>
                      ...
                      className={`px-4 py-2 rounded-xl border arabic-text text-lg font-semibold word-chip transition-all
                        ${
                          used
                            ...
                            : darkMode
                              ? "bg-white/5 border-white/15 text-slate-200 hover:bg-teal-500/10 hover:border-teal-500/30 hover:text-teal-300"
          ...
          >
            ...
            <textarea
              ...
              placeholder={
                lang === "id"
                  ? "Ketik jawaban dalam bahasa Arab..."
                  : "Type your Arabic answer here..."
              }
              dir="rtl"
              rows={3}
            ...
            <p className={`text-xs ${subtext} mt-2`}>
              {lang === "id"
                ? `Jawaban: ${q.fullSentence}`
                : `Answer: ${q.fullSentence}`}
            </p>
          </div>
        )}
        ...
        {checked && (
          <div
            className={`${card} border rounded-2xl p-5 mb-4 animate-fade-in-up ${isCorrect ? "border-teal-500/50" : "border-red-500/50"}`}
          >
            <div className="flex items-start gap-3">
              <span className="text-3xl">{isCorrect ? "🎉" : "😅"}</span>
              <div>
                <p
                  className={`font-black text-base ${isCorrect ? "text-teal-400" : "text-red-400"}`}
                >
        ...
        <div className="animate-fade-in-up">
          {!checked ? (
            <button
              onClick={handleCheck}
              disabled={
                mode === "click" ? sentence.length === 0 : !typedAnswer.trim()
              }
